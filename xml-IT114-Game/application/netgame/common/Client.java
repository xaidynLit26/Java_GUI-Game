...
abstract public class Client {
    ...
    public void disconnect() {
        if (!connection.closed)
            connection.send(new DisconnectMessage("[xml] Goodbye Hub"));
    }
    ...
    public void send(Object message) {
        ...
        if (connection.closed)
            throw new IllegalStateException("[xml] Message cannot be sent because the connection is closed.");
    ...
    private  class ConnectionToHub {
        ...
        ConnectionToHub(String host, int port) throws IOException {
            ...
            try {
                ...
                throw new IOException("[xml] Illegal response from server.");
            }
        ...
        private class SendThread extends Thread {
            public void run() {
                System.out.println("[xml] Client send thread started.");
                try {
                    while ( ! closed ) {
                    ...
                    if ( ! closed ) {
                        closedByError("[xml] IO error occurred while trying to send message.");
                        System.out.println("[xml] Client send thread terminated by IOException: " + e);
                    }
                }
                catch (Exception e) {
                    if ( ! closed ) {
                        closedByError("[xml] Unexpected internal error in send thread: " + e);
                        System.out.println("\n[xml] Unexpected error shuts down client send thread:");
                        e.printStackTrace();
                    }
                }
                finally {
                    System.out.println("[xml] Client send thread terminated.");
                }
            }
        }
        ...
        private class ReceiveThread extends Thread {
            public void run() {
                System.out.println("[xml] Client receive thread started.");
                try {
                    while ( ! closed ) {
                    ...
                    if ( ! closed ) {
                        closedByError("[xml] IO error occurred while waiting to receive  message.");
                        System.out.println("[xml] Client receive thread terminated by IOException: " + e);
                    }
                }
                catch (Exception e) {
                    if ( ! closed ) {
                        closedByError("[xml] Unexpected internal error in receive thread: " + e);
                        System.out.println("\n[xml] Unexpected error shuts down client receive thread:");
                        e.printStackTrace();
                    }
                }
                finally {
                    System.out.println("[xml] Client receive thread terminated.");
                }
            }
        }
