...
public class Hub {
    ...
    public Hub(int port) throws IOException {
        ...
        System.out.println("[xml] Listening for client connections on port " + port);
            ...
            public void run() {
               while (true) {
                   try {
                       ...
                       System.out.println("[xml] Exception while handling received message:");
                       e.printStackTrace();
                   }
               }
           }
    ...
    public void shutDownHub() {
        ...
        sendToAll(new DisconnectMessage("[xml] *shutdown*"));
        try {
            Thread.sleep(1000);
        }
    ...
    synchronized public void sendToAll(Object message) {
        if (message == null)
            throw new IllegalArgumentException("[xml] Null cannot be sent as a message.");
        if ( ! (message instanceof Serializable) )
            throw new IllegalArgumentException("[xml] Messages must implement the Serializable interface.");
        for (ConnectionToClient pc : playerConnections.values())
            pc.send(message);
    }
    ...
    synchronized private void acceptConnection(ConnectionToClient newConnection) {
        ...
        sendToAll(sm);
        playerConnected(ID);
        System.out.println("[xml] Connection accepted from client number " + ID);
    }
    ...
    synchronized private void clientDisconnected(int playerID) {
        if (playerConnections.containsKey(playerID)) {
            ...
            System.out.println("[xml] Connection with client number " + playerID + " closed by DisconnectMessage from client.");
        }
    }
        ...
        public void run() {
            try {
                while ( ! shutdown ) {
                    ...
                    if (shutdown) {
                        System.out.println("[xml] Listener socket has shut down.");
                        break;
                    }
                ...
                if (shutdown)
                    ...
                    System.out.println("[xml] Listener socket has been shut down by error: " + e);
            }
        }
    }
            ...
            public void run() {
                try {
                    ...
                    if ( ! "Hello Hub".equals(handle) )
                        throw new Exception("[xml] Incorrect hello string received from client.");
                    ...
                    System.out.println("\n[xml] Error while setting up connection: " + e);
                    e.printStackTrace();
                    return;
                }
                try {
                    ...
                    if (! closed) {
                        closedWithError("[xml] Error while sending data to client.");
                        System.out.println("[xml] Hub send thread terminated by IOException: " + e);
                    }
                }
                catch (Exception e) {
                    if (! closed) {
                        closedWithError("[xml] Internal Error: Unexpected exception in output thread: " + e);
                        System.out.println("\n[xml] Unexpected error shuts down hub's send thread:");
                        e.printStackTrace();
                    }
                }
            }
        ...
        private class ReceiveThread extends Thread {
            public void run() {
                try {
                    ...
                    if (! closed) {
                        closedWithError("[xml] Error while reading data from client.");
                        System.out.println("[xml] Hub receive thread terminated by IOException: " + e);
                    }
                }
                catch (Exception e) {
                    if ( ! closed ) {
                        closedWithError("[xml] Internal Error: Unexpected exception in input thread: " + e);
                        System.out.println("\n[xml] Unexpected error shuts down hub's receive thread:");
                        e.printStackTrace();
                    }
                }
            }
